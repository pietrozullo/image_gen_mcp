...
### Testing with mcp-use Inspector

You can test the application using the mcp-use development server with debug mode enabled:

```python
# In server.py, change the run call to:
...
Debug mode provides these endpoints:
- `/openmcp.json` - server discovery metadata
- `/docs` - interactive documentation
- `/inspector` - monitoring and debugging interface

This provides a convenient web interface where you can directly test the image generation tool without needing to use Claude or another MCP client.
